...
class TestDirectAliases:
    ...
    def test_reverse_lookup_by_model_id(self, monkeypatch):
        ...
        result = resolve_alias("kimi-k2.5", "custom")
        assert result is not None
        provider, model, alias = result
    ...
    def test_reverse_lookup_case_insensitive(self, monkeypatch):
        ...
        result = resolve_alias("glm-4.7", "custom")
        assert result is not None
        assert result[1] == "GLM-4.7"

    def test_reverse_lookup_never_crosses_provider_boundary(self, monkeypatch):
        """A model ID must not inherit another provider alias's endpoint."""
        from hermes_cli.model_switch import DirectAlias, resolve_alias
        import hermes_cli.model_switch as ms
        ...
        assert resolve_alias("shared-model", "openrouter") is None
